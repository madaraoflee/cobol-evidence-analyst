...
帶到公司電腦時，可使用[新版便攜包](../../.poc-data/delivery/cobol-lens-workbench-2026-09-14.zip)，完整解壓後閱讀 `START-HERE.md`，執行根目錄的 `START-WORKBENCH.bat`。需要 Python 3.10+；包內沒有 Key、資料庫或測試源碼。
...
未接入任何源碼時，畫面提供明確標記的**示例預覽**。切換到本機源碼模式，輸入源碼與輸出目錄，才能建立當次真實索引；程式與數量會依服務回傳更新。建立索引始終離線；按「開始業務分析」才會使用已設定的公司模型接口，傳送問題及有限源碼證據，按鈕旁會明示這個用途。尚未配置時先顯示連線設定說明。公司 API 設定從啟動終端的環境變數讀取，服務不回傳 API Key；接口已設定也不等於能力已驗證。
...
另以模擬模型回應和真實索引／證據驗證了回答呈現、工具記錄、換題後清空舊結果與證據不足狀態，並核對瀏覽器下載的 Markdown 摘要。全套 685 項測試通過；便攜包獨立解壓後的入口、離線索引及 HTTP 服務檢查通過。

...
> 当前执行进度与逐项任务请看[任务计划与进度总表](./12-task-plan-and-progress.md)。本文保留早期路线设想与技术切片记录；早期工期不是当前承诺，P3-A～F 也不表示产品 P3 已通过。
...
最新 P3-F 更新：显式语法子集的局部异常/溢出控制流及状态路径已实现，能检查错误后的模型退出输出和清零后的覆写。5 程序新样例的所选异常预期通过，独立反例正确检出输出 7；完整跨程序业务执行仍未证明。下一退出门槛是被调程序摘要与调用上下文组合，再扩循环与共享状态；详见 [P3-F 报告](./reports/2026-09-08-p3f-exception-path-feasibility.md)。下方为先前切片记录。

P3-E 已实现完整静态调用点链上下文，7 程序重复调用样例展开为 12 个上下文，48 项源码身份/映射验收通过。状态链不会跨调用分支拼接，内容复制或缺失链条不升级为入口回写候选。详见 [P3-E 报告](./reports/2026-09-08-p3e-call-context-feasibility.md)。

P3-D 已实现受支持的 CALL USING/LINKAGE 参数及固定组成员对应，并加入只读错误路径审查与 14 程序复杂夹具。主样例有 205 条参数对应、92 条候选回写；配置目标和完整控制流仍未证明。详见 [P3-D 报告](./reports/2026-09-08-p3d-cross-program-error-flow.md)。

...
> 核心判断：这个产品不是“把代码切块后交给大模型聊天”，而是一套以版本化源码及获准配置、数据定义为事实依据的自动调查系统。它先恢复程序与框架控制来源，再用混合 RAG 找到调查入口，沿确定性关系收集证据，最后才让大模型把证据翻译成业务语言；配置声明与运行验证不能混同。
...
> 2026-09-12 框架主线进展：F01 保留控制 COPY 原始行与包含链，F02 将派生源码送入现有 CFG 框架模式并推进受支持的子程序调用与参数返回。F03 的单数据集契约子集已模拟筛选、游标、锁、事务和检查点恢复；真实 Data Set/LF、生成接口、调度与配置仍待同版资料适配。所选场景可到达模型终态，不等于全部路径、真实运行或业务问答已完成；新工具尚未接入 Agent。操作见[框架路径与运行契约](./docs/14-framework-paths-and-runtime-contracts.md)，决策见 [ADR-0009](./docs/adr/0009-contract-conditional-framework-paths.md)。
...
统一 IR 与完整 Agent 工具契约保留为未来演进边界，不作为 POC 前置条件。公开侧现已完成离线画像、SQLite/FTS5 结构索引、四个受限调查工具、公司 API capability probe 和最多六步的 Agent 骨架；P3-B 已把独立 `COMPUTE` 断言核验接入 CALC-01 的 4 次真实工具调用离线闭环。当前可证明目标字段、算式 token 顺序和舍入关键字与单段源码一致，不能据此证明执行路径、最终值或任意自然语言结论。下一退出门槛是在公司批准环境验收真实 API，并以评测样本约束后续断言类型扩展。真实公司代码画像、DXC 方言适配和业务金标准只能在公司批准环境内执行。DDL/DDS、Job Schedule、DB File、Item Table 和运行数据未下载，必须作为回答边界；Embedding、图服务与 LangGraph 继续暂缓。

...
const preview = {
  ...
  evidence:[
    {evidence_id:'preview-01',title:'分期保費計算',relative_path:'programs/premium-calc.cbl',start_line:42,end_line:47,integrity:'ILLUSTRATIVE',source_text:'COMPUTE INSTALMENT-PREMIUM\n    ROUNDED =\n    ANNUAL-PREMIUM\n    * PAYMENT-FACTOR\nEND-COMPUTE.\nMOVE INSTALMENT-PREMIUM TO OUT-PREMIUM.'},
    {evidence_id:'preview-02',title:'年繳保費組成',relative_path:'programs/premium-calc.cbl',start_line:30,end_line:35,integrity:'ILLUSTRATIVE',source_text:'COMPUTE ANNUAL-PREMIUM =\n    BASE-PREMIUM\n    + RIDER-PREMIUM\n    - PREMIUM-DISCOUNT\nEND-COMPUTE.\nPERFORM GET-PAYMENT-FACTOR.'},
    {evidence_id:'preview-03',title:'繳費係數來源',relative_path:'programs/payment-factor.cbl',start_line:18,end_line:23,integrity:'ILLUSTRATIVE',source_text:'EXEC SQL\n    SELECT MODE-FACTOR\n    INTO :PAYMENT-FACTOR\n    FROM PAYMENT-OPTIONS\n    WHERE MODE-CODE = :PAYMENT-MODE\nEND-EXEC.'},
  ],
  relations:[
...
function statusLabel(status){return ({SUPPORTED_WITH_BOUNDARIES:'已核驗局部語句 · 保留邊界',CITATION_VERIFIED_ONLY:'引用已核對 · 解釋待核驗',ABSTAINED:'證據不足 · 尚未形成結論',NOT_READY:'尚未具備分析條件',SAFE_STOP:'調查已停止 · 需要處理',NETWORK_DISABLED:'尚未啟用模型問答',NOT_REQUESTED:'尚未提出業務問題',BLOCKED:'接入受阻',INDEX_READY:'源碼已就緒',NEEDS_ATTENTION:'源碼已接入 · 有待核對',PREPARING:'正在接入源碼'})[status] || status || '尚未分析';}
...
function renderChrome(){
  ...
  const titles={workbench:['分析工作台','讓業務邏輯，清晰可見。','從一個業務問題出發，沿著源碼找到可追溯的答案。'],sources:['源碼資料庫','先看清楚，分析了甚麼。','程式、來源與版本，都有跡可尋。'],history:['分析記錄','每一次分析，都有依據。','回看本次工作階段的問題、來源快照與結論。']};
  ...
  $('connection-status').innerHTML=`<i></i>${state.connected?'本機服務已啟動':'介面預覽 · 本機服務未啟動'}`;
  $('config-dot').classList.toggle('ready',state.apiConfigured);
  $('project-title').textContent=demo?'保費計算 · 合成示例':(state.project?.source?.split(/[\\/]/).filter(Boolean).pop() || '尚未接入本機源碼');
  const badge=$('source-badge');badge.textContent=demo?'示例資料':(state.busy?'更新中':(d?statusLabel(d.runner_status):'等待接入'));
  badge.className=`badge ${demo?'preview':d?.runner_status==='BLOCKED'?'warning':sourceUsable()?'positive':'neutral'}`;
  $('project-description').textContent=demo?'8 個檔案 · 6 個程式定義 · 2 個 COPYBOOK':d?`${formatNumber(files?.decoded)} 個檔案 · ${formatNumber(d.program_count)} 個程式定義 · ${formatNumber(d.build_report?.diagnostics?.copybook_count)} 個 COPYBOOK`:'選擇源碼與結果資料夾，建立自己的分析範圍';
  $('snapshot-info').innerHTML=icon('layers')+(demo?'介面示例 · 非實際分析':state.project?.snapshot_id?`快照 ${escapeHTML(state.project.snapshot_id.replace('sha256:','').slice(0,8))}`:'尚未建立快照');
  $('nav-count').textContent=demo?'8':formatNumber(files?.decoded);
  $('recent-title').innerHTML=demo?'分期保費計算邏輯<small>合成示例 · 介面預覽</small>':`${escapeHTML(d?.question || '尚未建立業務問答')}<small>${d?'本機源碼 · 本次工作階段':'接入源碼後開始'}</small>`;
  $('footer-scope').textContent=demo?'介面示例 · 不代表公司業務結果':'本機源碼 · 問題完整性仍需覆核';
...
function actualAnswer(){
  ...
  if(draftChanged())return `<div class="answer-empty">${icon('spark')}<h2>準備分析新的問題</h2><p>問題或調查起點已更改。發起分析後，這裡會呈現新問題的結果。<br>上一題摘要保留在分析記錄中。</p></div>`;
  if(!answer || !answer.claims?.length){
    const failed=answer || ['NOT_READY','SAFE_STOP','BLOCKED','NETWORK_DISABLED'].includes(status);
    return `<div class="answer-empty">${icon(failed?'info':'spark')}<h2>${escapeHTML(statusLabel(status))}</h2><p>${failed?'保留現有資料與阻斷原因。處理缺口後，可重新發起分析。':'源碼已就緒。選擇調查起點，輸入業務問題，即可從當前源碼開始。'}</p></div>${renderBoundaries(answer?.boundaries || d?.messages || [])}${state.project?.agent?.reason_code?`<p class="source-note">診斷代碼：${escapeHTML(state.project.agent.reason_code)}</p>`:''}`;
  }
  const refs=currentRefs();
  return `<div class="real-answer"><div class="answer-state-row"><span class="badge neutral">${icon('check')}${escapeHTML(statusLabel(answer.status))}</span><small>${refs.length} 段引用</small></div><h2>來自當前源碼的分析結果</h2><p class="answer-intro">以下陳述保留實際核驗狀態。點選引用，可回到本次快照的源碼位置。</p><div class="steps">${answer.claims.map((claim,i)=>`<div class="business-step"><span class="step-number">${String(i+1).padStart(2,'0')}</span><div><p class="claim-text">${escapeHTML(claim.claim || claim.text || '')}${(claim.evidence_ids || []).map(id=>cite(id,String(Math.max(1,refs.findIndex(r=>r.evidence_id===id)+1)))).join('')}</p><small class="field-hint">${escapeHTML(({code_fact:'源碼事實',business_inference:'業務推測 · 未核驗',open_question:'待確認問題 · 未形成結論'})[claim.kind] || '候選解讀')} · ${escapeHTML(({supported:'局部語句已核驗',citation_verified_only:'引用已核對，語義待核驗',unsupported:'證據尚不支持'})[claim.support_status] || '語義未核驗')}</small></div></div>`).join('')}</div>${renderBoundaries(answer.boundaries || [])}<div class="answer-footnote">${icon('info')}問題相關性與完整性尚未核驗；局部語句通過不代表整條業務流程已驗證。</div></div>`;
}
function renderBoundaries(items){if(!items.length)return '';return `<div class="boundary-box">${icon('alert')}<div><strong>待確認與分析邊界</strong>${items.map(b=>`<p>${escapeHTML(boundaryText(b))}</p>`).join('')}</div>`;}
function relationsView(){
  const edges=currentRelations();return `<h2>沿著程式，回查關係。</h2><p class="graph-subtitle">${state.mode==='demo'?'此處是預先編排的關係示例。':'以下來自當前源碼的靜態呼叫與 COPY 關係，不表示執行順序或實際可達。'}${state.project?.relations?.truncated && state.mode==='real'?' 關係數量已達顯示上限。':''}</p>${edges.length?edges.slice(0,40).map(edge=>`<div class="relation-row"><div class="relation-program">${escapeHTML(edge.source_program || edge.relative_path || '來源')}<span class="relation-meta">${escapeHTML(edge.relation_type==='INCLUDES_COPY'?'引入定義':'呼叫來源')}</span></div><div class="relation-link">${escapeHTML(edge.relation_type)}</div><div class="relation-program">${escapeHTML(edge.target_name || '未解析目標')}<span class="relation-meta">${escapeHTML(({confirmed:'源碼關係已確認',candidate:'候選關係',unresolved:'未能確認',illustrative:'示例關係'})[edge.status] || edge.status)}</span></div>${edge.evidence_id?cite(edge.evidence_id,'↗'):''}</div>`).join(''):'<div class="answer-empty"><p>本次沒有可展示的呼叫或 COPY 關係；不能據此推斷不存在依賴。</p></div>'}${edges.length>40?'<p class="source-note">目前顯示前 40 項關係。完整回傳資料可在匯出檔中查看。</p>':''}`;
}
function traceView(){
  if(draftChanged())return `<div class="answer-empty">${icon('clock')}<h2>新問題尚未執行調查</h2><p>問題或調查起點已更改。發起分析後，這裡會顯示新問題的工具記錄。</p></div>`;
  if(state.mode==='demo')return `<h2>每一步，留下可核對的依據。</h2><p class="graph-subtitle">以下是介面呈現方式示例，並非已執行的模型調查記錄。</p>${[['定位分析起點','從程式目錄及名稱，選定本次要調查的範圍。'],['核對計算與依賴','檢視欄位讀寫及呼叫關係，收集相關源碼引用。'],['集中讀取證據','閱讀已發現的源碼片段，逐項綁定結論與未決事項。']].map(([title,desc])=>`<div class="trace-row"><h3>${title}<span class="badge preview">示例步驟</span></h3><p>${desc}</p></div>`).join('')}`;
  const trace=result()?.tool_trace || [];const labels={search_code:'定位程式與欄位',inspect_symbol:'檢視定義與依賴',trace_relations:'追蹤源碼關係',read_evidence:'讀取源碼證據'};
  return `<h2>本次調查記錄</h2><p class="graph-subtitle">只顯示實際工具活動與狀態，不包含模型私有推理。</p>${trace.length?trace.map((item,i)=>{const name=item.tool || item.action || item.tool_name;const outcome=({SUCCEEDED:'已執行',FAILED:'執行失敗',REJECTED:'參數未通過核對',POLICY_REJECTED:'調查範圍受限',SNAPSHOT_REJECTED:'快照核對失敗',ATTEMPTED:'已嘗試'})[item.outcome] || item.outcome || '狀態未提供';return `<div class="trace-row"><h3>${String(i+1).padStart(2,'0')} · ${escapeHTML(labels[name] || name || '工具活動')}</h3><p><code>${escapeHTML(name)}</code> · ${escapeHTML(outcome)}${item.result?.status?' · '+escapeHTML(item.result.status):''}</p><details class="trace-details"><summary>查看工具資料</summary><pre>${escapeHTML(JSON.stringify(item.arguments || {},null,2))}</pre></details></div>`;}).join(''):'<div class="answer-empty"><p>尚未執行模型調查。建立索引不會產生模型工具記錄。</p></div>'}`;
}
function codeMarkup(span){
  if(!span)return '<div class="answer-empty"><p>選擇一段引用，查看源碼內容。</p></div>';
...
function evidencePanel(){
  ...
  return `<aside class="evidence-panel" aria-label="源碼證據"><div class="evidence-heading"><span>${icon('code')}</span><h2>源碼證據</h2><small>${String(refs.length).padStart(2,'0')}</small></div><div class="evidence-summary"><span>${demo?'示例引用':'本次回答引用'}</span><span>${demo?'非實際源碼驗收':'點選檢視原文'}</span></div><div class="evidence-list">${refs.length?refs.map((ref,i)=>`<button class="evidence-item ${state.selectedEvidence===ref.evidence_id?'selected':''}" data-evidence="${escapeHTML(ref.evidence_id)}"><span class="evidence-number">${String(i+1).padStart(2,'0')}</span><span><strong>${escapeHTML(ref.title || ref.relative_path?.split('/').pop() || '源碼引用')}</strong><small>${escapeHTML(ref.relative_path?.split('/').pop() || '')} · L${Number(ref.start_line)}–${Number(ref.end_line)}</small></span>${icon('chevron')}</button>`).join(''):'<div class="answer-empty"><p>分析結論的引用<br>將在這裡顯示。</p></div>'}</div><div class="code-header"><span>${icon('document')} ${escapeHTML(span?.relative_path?.split('/').pop() || '尚未選擇證據')}</span><span>COBOL</span></div><div class="code-body" aria-label="源碼原文">${codeMarkup(span)}</div><div class="evidence-location"><div>${icon('folder')}<span>${escapeHTML(span?.relative_path || '從引用清單選擇')}</span></div><div>${icon('layers')}<span>${demo?'介面示例':`快照 ${(state.project?.snapshot_id || '').replace('sha256:','').slice(0,12) || '未建立'}`}</span></div>${span?`<span class="badge ${demo?'preview':span.integrity==='VALID'?'positive':'warning'}">${icon(demo?'info':'check')}${demo?'示例內容':span.integrity==='VALID'?'快照證據完整性有效':'完整性未確認'}${span.span_truncated?' · 已截斷':''}</span>`:''}</div><div class="evidence-scope"><b>讓結論可被覆核</b><br>${demo?'選擇本機源碼後，此面板會展示實際文件、行號與證據。':'引用有效只表示證據可核對，不代表業務含義或所有執行路徑已獲證明。'}</div></aside>`;
}
function renderWorkbench(){
  if(state.mode==='real' && state.busy)return `<section class="answer-card"><div class="loading" role="status"><div class="spinner"></div><h2>${state.jobKind==='question'?'正在調查這次的業務問題':'正在接入你的源碼'}</h2><p>請保持此頁開啟，完成後會自動更新。<br>你可以稍後回到分析記錄查看結果。</p><span class="job-phase">${state.jobKind==='question'?'更新索引 → 檢查接口 → 調查源碼':'讀取源碼 → 建立索引 → 核對範圍'}</span></div></section>`;
  if(state.mode==='real'&&!diagnosis())return `<section class="empty-panel"><div class="empty-icon">${icon('folder')}</div><div class="eyebrow">YOUR CODE. YOUR CONTEXT.</div><h2>從自己的源碼，開始。</h2><p>接入本機 COBOL 程式與 COPYBOOK，確認分析範圍，<br>再用業務語言提出問題。</p><div class="empty-actions"><button class="button primary" data-connect>${icon('plus')}接入本機源碼</button><button class="button secondary" data-demo>先查看介面示例</button></div></section>`;
...
function sourceRows(){return currentPrograms().filter(p=>(p.program_name+' '+p.relative_path).toLowerCase().includes(state.filter.toLowerCase())).map(p=>`<tr><td>${escapeHTML(p.program_name)}</td><td>${escapeHTML(p.relative_path)}</td><td>L${Number(p.start_line)}</td><td><span class="badge ${state.mode==='demo'?'preview':'neutral'}">${state.mode==='demo'?'介面示例':'已識別定義'}</span></td><td><button data-program="${escapeHTML(p.program_name)}">分析程式 ${icon('arrow')}</button></td></tr>`).join('');}
function dependencyGaps(){const gaps=diagnosis()?.unresolved_dependencies || [];if(!gaps.length)return '';return `<section class="data-card dependency-gaps"><div class="data-card-heading"><h2>待確認的程式與 COPY 依賴</h2><span class="badge warning">${formatNumber(gaps.length)} 項</span></div><div class="table-wrapper"><table><thead><tr><th>目標名稱</th><th>關係</th><th>來源檔案</th></tr></thead><tbody>${gaps.slice(0,20).map(g=>`<tr><td>${escapeHTML(g.target_name || '動態目標')}</td><td>${escapeHTML(g.relation_type)}</td><td>${escapeHTML(g.relative_path)}</td></tr>`).join('')}</tbody></table></div><div class="table-count">${gaps.length>20?'目前顯示前 20 項；完整清單保留在匯出 JSON。':'請補齊對應源碼；動態目標可能仍需設定資料確認。'}</div></section>`;}
function renderSources(){
  ...
  return `<div class="source-stats"><div class="source-stat"><span>實際讀取檔案</span><strong>${demo?'8':formatNumber(files?.decoded)}</strong><small>${demo?'介面展示數據':`${formatNumber(files?.candidate)} 個候選檔案`}</small></div><div class="source-stat"><span>程式定義</span><strong>${formatNumber(programs.length)}</strong><small>依 PROGRAM-ID 識別</small></div><div class="source-stat"><span>COPYBOOK 定義</span><strong>${demo?'2':formatNumber(d?.build_report?.diagnostics?.copybook_count)}</strong><small>${demo?'介面展示數據':`${formatNumber(files?.unreadable_or_binary)} 個候選檔案未能讀取`}</small></div></div><section class="data-card"><div class="data-card-heading"><h2>程式目錄</h2><input id="program-filter" aria-label="搜尋程式名稱或路徑" placeholder="搜尋程式名稱或路徑" value="${escapeHTML(state.filter)}"></div><div class="table-wrapper"><table><thead><tr><th>PROGRAM-ID</th><th>來源路徑</th><th>定義行</th><th>識別狀態</th><th></th></tr></thead><tbody id="program-rows">${sourceRows()}</tbody></table></div><div class="table-count">${formatNumber(programs.length)} 個程式定義 · ${demo?'合成示例':'目前源碼快照'}</div></section>${!demo?`<div class="source-note"><strong>源碼位置</strong><br>${escapeHTML(state.project.source)}<br><strong>本次快照</strong><br>${escapeHTML(state.project.snapshot_id || '未建立有效快照')}</div>${renderBoundaries(d?.messages || [])}`:''}`;
}
function renderHistory(){
  if(state.mode==='demo')return `<section class="data-card"><div class="data-card-heading"><h2>介面示例記錄</h2><span class="badge preview">非實際分析</span></div><div class="history-card">${icon('document')}<div><h3>${preview.question}</h3><p>預先編排的介面內容 · 3 段示例引用</p></div><button data-demo-result>查看示例 ${icon('arrow')}</button></div></section>`;
  return `<section class="data-card"><div class="data-card-heading"><h2>本次工作階段</h2><span class="badge neutral">${state.history.length} 項</span></div>${state.history.length?state.history.map((item,i)=>`<div class="history-card">${icon('document')}<div><h3>${escapeHTML(item.diagnosis?.question || '源碼接入')}</h3><p>${escapeHTML(item.source?.split(/[\\/]/).pop())} · ${escapeHTML(statusLabel(item.agent?.agent_result?.status || item.diagnosis?.question_status))} · ${escapeHTML(item.diagnosis?.generated_at_utc || '')}</p></div><button data-history="${i}">查看摘要</button></div>`).join(''):'<div class="empty-panel"><h2>尚未有分析記錄</h2><p>本次工作階段完成的分析會列在這裡；完整結果同時保存在你指定的資料夾。</p></div>'}</section>`;
}
...
async function api(path,options={}){
  ...
  let value;try{value=await response.json();}catch{throw new Error('本機服務未回傳有效資料。請重新啟動服務。');}
  if(!response.ok)throw new Error(value.error?.message || value.error?.code || '本機服務暫時無法完成操作。');
  return value;
}
...
function openSource(){
  ...
  if(!state.connected){$('source-error').textContent='請先以 python poc/web_app.py 啟動本機服務，再透過服務地址開啟此頁。';$('source-error').hidden=false;}
...
async function pollJob(jobId){
  try{const job=await api(`/api/jobs/${encodeURIComponent(jobId)}`);if(state.jobId!==jobId)return;
    ...
    if(job.status==='FAILED')state.error=job.error?.message || '接入未完成，請檢查資料範圍及設定。';
...
async function loadEvidence(id){
  ...
  try{const data=await api(`/api/evidence?id=${encodeURIComponent(id)}`);if(ticket!==state.evidenceTicket || state.mode!=='real' || runId!==state.project?.run_id)return;
    if(data.run_id && runId && data.run_id!==runId)throw new Error('源碼快照已更新，請重新選擇本次引用。');
    const span=data.spans?.find(s=>s.evidence_id===id);if(!span || span.integrity!=='VALID')throw new Error('這段證據未通過快照完整性核對，暫不展示原文。');
...
document.addEventListener('click',event=>{
  ...
  if(target.dataset.question){if(state.mode==='demo'){toast('這是介面示例。請接入本機源碼，才能調查自己的問題。');return;}state.question=target.dataset.question;render();$('question-input')?.focus();}
  if(target.dataset.program){state.entry=target.dataset.program;state.question=state.mode==='demo'?preview.question:`請解釋 ${target.dataset.program} 的主要處理步驟、輸入輸出及呼叫。`;state.page='workbench';state.tab='answer';render();}
  ...
  if(target.hasAttribute('data-history')){const item=state.history[Number(target.dataset.history)];download(markdownSummary(item),'text/markdown;charset=utf-8','source-analysis-history.md');toast('已匯出該次摘要；目前源碼範圍保持不變。');}
...
document.addEventListener('submit',async event=>{
  if(event.target.id==='question-form'){
    ...
    if(state.mode==='demo'){if(state.question!==preview.question || state.entry!=='PREMIUM-CALC'){toast('示例只展示預先編排的保費問題。接入本機源碼後，可分析自己的程式。');return;}state.tab='answer';render();toast('目前為介面示例，未發起模型請求。');return;}
    if(!state.question.trim()){toast('請先輸入一個業務問題。');return;}
    if(!sourceUsable()){toast('請先成功接入本機源碼。');return;}
...
function showSettings(){$('settings-description').textContent=state.apiConfigured?'接口設定已提供，發起問答時會進行實際能力探測。':'源碼索引可離線使用。模型問答需要啟動服務時提供接口設定。';$('settings-local').textContent=state.connected?'已啟動':'尚未啟動';$('settings-api').textContent=state.apiConfigured?'已提供 · 待實際驗證':'尚未提供';$('settings-dialog').showModal();}
...
$('export-markdown').addEventListener('click',()=>{download(markdownSummary(),'text/markdown;charset=utf-8',state.mode==='demo'?'interface-preview-summary.md':'source-analysis-summary.md');$('export-dialog').close();toast('已匯出摘要，包含資料模式與分析邊界。');});
$('export-json').addEventListener('click',()=>{download(JSON.stringify(state.mode==='demo'?{mode:'illustrative_preview',model_called:false,...preview}:{mode:'local_source',...state.project},null,2),'application/json;charset=utf-8',state.mode==='demo'?'interface-preview.json':'source-analysis.json');$('export-dialog').close();toast('已匯出本次資料。');});
render();initialize();
